...
class TweetsGetter(object):
    ...
    def __init__(self):
        ...
        #self.session = OAuth1Session(bearer_token=BEARER_TOKEN)
...
if __name__ == '__main__':
    ...
    def remove_emoji(src_str):
      return ''.join(c for c in src_str if c not in emoji.UNICODE_EMOJI).replace('\n','')
    outfile = open('tweet_data_' + d.strftime('%Y%m%d%H%M') + '.csv', 'w',encoding='CP932', errors='ignore')
    ...
    for screen_name in salon_members_list:
        ...
        cnt = 0
        for tweet in getter.collect(total = 3200):
            cnt += 1
            outfile.write(str(remove_emoji(screen_name))+","+ str(tweet['created_at'])+","+str(remove_emoji(tweet['text'])).replace(',','')+","+str(tweet['retweet_count'])+","+str(tweet['favorite_count'])+","+str(tweet['in_reply_to_screen_name'])+"\n")
          #  print(tweet['text'])
        del getter
    outfile.close()
